...
#' @export
replaceESEData <- function(cxnObj = NULL, source_df= NULL, target_schema = "groundfish", target_table = NULL){
  ...
  tryDelete = tryCatch(
    {
      DBI::dbExecute(cxnObj$channel, paste0("DELETE FROM ",target_schema,".", target_table))
    },
    error=function(cond){
      message(cond)
      return(-1)
  ...
  if (tryDelete == -1){
    stop("Could not remove old data from ",target_schema,".", target_table)
  }else{
    message("deleted old data")
  }
  
  tryWrite = tryCatch(
    {
      ROracle::dbWriteTable(conn = cxnObj$channel, schema=target_schema, value = source_df, 
                            name = target_table, row.names = FALSE, overwrite=T)
    },
    error=function(cond){
      message(cond)
      return(-1)
  ...
  if (tryWrite == -1) {
    stop("Could not write ", deparse(substitute(source_df)) ," to ",target_schema,".", target_table)
  }else{
    message("new data written")
  }
}
