...
redistributeMixedCatch <- function(catch=NULL, basket = NULL, quiet=T){
  ...
  for (i in 1:length(parentIDs)){
    ...
    parentBaskets <- merge(thisParentC, thisParentB[, !names(thisParentB) %in% c("SPEC")] , all.y=T, by.x=c("MISSION", "SETNO", "SIZE_CLASS","catch_id"),
                           by.y=c("MISSION", "SETNO", "SIZE_CLASS","catch_id"))
    ...
    childBaskets <- merge(thisChildC, thisChildB[, !names(thisChildB) %in% c("SPEC")] , all.y=T, by.x=c("MISSION", "SETNO", "SIZE_CLASS","catch_id"),
                          by.y=c("MISSION", "SETNO", "SIZE_CLASS","catch_id"))
    ...
    newCatch  <- childBaskets[,c("MISSION", "SETNO", "SPEC", "catch_id","NOTE", "UNWEIGHED_BASKETS", "NUM_calc", "is_parent", "parent_catch_id", "SIZE_CLASS")]
...
redistributeMixedCatch2 <- function(catch=NULL, basket = NULL){
  if(F){
    x <- list()
    ...
    for (i in 1:length(parentIDs)){
      ...
      sampled <- merge(sampC, sampB[, !names(sampB) %in% c("SPEC")] , all.y=T, by.x=c("MISSION", "SETNO", "SIZE_CLASS","id"),
                       by.y=c("MISSION", "SETNO", "SIZE_CLASS","basket_catch_id"))
      sampled$basket_catch_id <- sampled$id
      ...
      if (nrow(sampled)!=nrow(unsampled))browser()
      #if(abs(round(sum(sampled$BASKET_WEIGHT)-sum(unsampled$BASKET_WEIGHT),1))>0.5)
      newRecs <- sampled
