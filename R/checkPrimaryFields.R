...
checkPrimaryFields <- function(primaryTable = NULL){
  ...
  # if (primaryTable %in% c("ESE_CATCHES","ESE_BASKETS")) {
  #   toCheck <- toCheck[!toCheck %in% c("ESE_SPECIMENS", "ESE_LV1_OBSERVATIONS")]
  #   if(primaryTable == "ESE_CATCHES") message("Records from ESE_SPECIMENS or ESE_LV1_OBSERVATIONS must be in ESE_CATCHES")
  # }
  ...
  if (primaryTable %in% c("ESE_SETS")) message("\t(ESE_SETS can contain sets that are not 'Type 1', so ESE_CATCHES, ESE_BASKETS, ESE_SPECIMENS 
        and ESE_LV1_OBSERVATIONS do not necessarily have to contain all of the SETNOs found in ESE_SETS)")
  for (t in 1:length(toCheck)){
    ...
    theChk <- setdiff(thisTabKeys,primary)
    theChkRev <- setdiff(primary, thisTabKeys)
    if (primaryTable %in% c(ESE_CATCHES)) theChkRev <- NA
    ...
    if (length(theChkRev)>0) {
      
      message("Records exist in ", primaryTable," that are NOT found in ", toCheck[t])
      if (primaryTable == "ESE_SETS"){
        message("It is likely these are due to the presence of invalid sets")
      }else{
        browser()
      }
